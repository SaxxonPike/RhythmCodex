...
[Service]
public sealed class PsxGaussianInterpolation : IPsxGaussianInterpolation
{
    private static readonly Lazy<ReadOnlyMemory<short>> Table = new(() => new short[]
    {
    ...
    public int Interpolate(ReadOnlySpan<float> input, Span<float> output, float position, float targetRatio)
    {
        ...
        while (position < input.Length && outOffset < output.Length)
        {
            ...
            var calculated = (Vector128.Sum(gauss) >> 15) / 32768f;
            output[outOffset++] = calculated;
            position = basePosition + outOffset * sourceRate;
            result++;
        }
    ...
    public float InterpolateOne(float s3, float s2, float s1, float s0, float position)
    {
        ...
        return (Vector128.Sum(gauss) >> 15) / 32768f;
    }
}
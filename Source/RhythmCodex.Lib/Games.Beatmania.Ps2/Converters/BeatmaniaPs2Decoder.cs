...
{
    public List<BeatmaniaPs2ChartSet> Decode(Func<string, Stream> openFile, BeatmaniaPs2FormatType type)
    {
        if (formatDatabase.GetForType(type) is not { } formatInfo)
        ...
        Dictionary<string, Stream> blobStreams = [];

        var result = new List<BeatmaniaPs2ChartSet>();
        ...
        try
        {
            ...
            foreach (var file in masterFileTable.Values.Select(x => x.File).Distinct())
                blobStreams[file] = openFile(file);
            ...
            foreach (var (songId, songInfo) in masterSongTable)
            {
                ...
                var chartMap = songInfo.Difficulties
                    ...
                    .ToDictionary(x => x.Index, x => (
                        ...
                        Bgm: x.Item.Bgm < 0 ? bgmMap.First().Value.SetId : x.Item.Bgm,
                        Keysounds: x.Item.Keysounds < 0 ? keyMap.First().Value.SetId : x.Item.Keysounds));
                ...
                {
                    chartSet.ChartMaps[chartIndex] = (BgmId: chartInfo.Bgm, KeysoundId: chartInfo.Keysounds);
                    chartInfo.Chart[NumericData.Id] = chartInfo.ChartId.Chart;
                ...
                foreach (var (_, keyInfo) in keyMap)
                    chartSet.Keysounds[keyInfo.SetId] = keyInfo.Data.Select(x =>
                    {
                        x.Value?[NumericData.Id] = x.Key;
                        return x.Value;
                    }).ToList()!;
            ...
            byte[] ReadFromBlob(int fileId)
            {
                ...
                var blobStream = blobStreams[info.File];
        ...
        finally
        {
            ...
            foreach (var (_, blobStream) in blobStreams)
                blobStream.Dispose();
        }

        Sound? ReadBgm(Stream stream, bool isOld) =>
            bgmDecoder.Decode(isOld
        ...
        Dictionary<int, Sound?> ReadKeysounds(Stream stream, bool isOld) =>
            (isOld
                ...
                : newKeysoundStreamReader.Read(stream))
            .Keysounds.ToDictionary(x => x.Index, keysoundDecoder.Decode);

    ...
    : BufferedCipherBase
{
    ...
    */
    ...
    {
        var updateOutputSize = GetUpdateOutputSize(1);

        var output = updateOutputSize > 0 ? new byte[updateOutputSize] : null;

        var outLen = ProcessByte(input, output, 0);
    ...
    */
    ...
    public override byte[] ProcessBytes(byte[] input, int inOff, int length)
    {
        ...
        var updateOutputSize = GetUpdateOutputSize(length);

        var output = updateOutputSize > 0 ? new byte[updateOutputSize] : null;

        var outLen = ProcessBytes(input, inOff, length, output, 0);
    ...
    {
        var outputSize = GetOutputSize(0);
        if (outputSize < 1)
        {
            Reset();
            return EmptyBuffer;
    ...
    public override byte[] DoFinal(byte[] input, int inOff, int inLen)
    {
        ...
        var outputSize = GetOutputSize(inLen);
        if (outputSize < 1)
        {
            Reset();
            return EmptyBuffer;
        ...
        var outLen = inLen > 0 ? ProcessBytes(input, inOff, inLen, output, 0) : 0;
